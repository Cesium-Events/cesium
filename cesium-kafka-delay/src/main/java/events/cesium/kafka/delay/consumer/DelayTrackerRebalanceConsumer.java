...
public class DelayTrackerRebalanceConsumer extends KafkaConsumer<Long, Long> {

    public DelayTrackerRebalanceConsumer(CesiumConfig cesiumConfig) {
        super(getKafkaConfig(cesiumConfig));
    }

    private static final Properties getKafkaConfig(CesiumConfig config) {
        Properties props = config.getProperties(DelayTrackerRebalanceConsumer.class, "KafkaConfig");
